...
class SrtGenerator {
    constructor(editorCtrl, rawData, path) {
        ...
        this.getElement = function (selector) {
            return this.getEditor().querySelector(selector);
        };
        ...
        this.getElements = function (selector) {
            return this.getEditor().querySelectorAll(selector);
        };
        ...
        this.clearActiveData = function () {
            let elems1 = this.getElements('.srt-list-item');
        ...
        this.updateAudioTimePosition = function () {
            if (this.audio != null) {
                ...
                if (this.scroll) {
                    let width = (this.getElement('.srt-map').parentNode.offsetWidth / 2);
                    this.getElement('.srt-map').scrollLeft = left - width;
                    let tooltipObj = this.getElement('.srt-time-position-pointer');
                    if (tooltipObj) {
                ...
                if (this.diplayedText != '' && this.diplayedTextLast != this.diplayedText) {
                    // text on
                    this.getElement('.text-display-inner').innerHTML = this.diplayedText;
                    this.diplayedTextLast = this.diplayedText;                   
                    this._onTextShow(index, this.diplayedText);
                }
                if (this.diplayedText == '' && this.diplayedTextLast != this.diplayedText) {
                    // text off
                    this.getElement('.text-display-inner').innerHTML = '';
                    this.diplayedTextLast = this.diplayedText;
                    this._onTextClear();
                }
            }
        ...
        this.showTextManual = function (index, text) {
            this.getElement('.text-display-inner').innerHTML = text;
            this.scrollToMiddle(index);
        ...
        this.resetZIndex = function () {
            let elems = this.getElements('.srt-controller');
            ...
            if (this.data.length > 1 && index < this.data.length - 1) {
                ...
                if (this.data[index].start + this.data[index].duration > this.data[nextIndex].start + this.data[nextIndex].duration) {
                    ...
                    if (this.data[index].start < this.data[nextIndex].start) {
                        ...
                        this.data[index].start = this.data[index].start;
                        this.data[index].duration = this.data[index].duration;
                        this.updateController(index);
                    }
        ...
        this.createControllerContainer = function (srtData, index) {
            ...
            controller.querySelector('.srt-text-edit').addEventListener('keydown', function (e) {
                if ((e.key === 'Enter' || e.keyCode === 13) && (!e.shiftKey && !e.altKey && !e.ctrlKey)) {
                    ...
                    let pos = _this.getCursorPos(textarea);
                    let text1 = '';
                    let text2 = '';
                    ...
                    let thisController = _this.closest(textarea, '.srt-list-item');
                    ...
                    let startMs1 = _this.convertTimestampToMills(start1);
                    let endMs1 = _this.convertTimestampToMills(end1);
                    ...
                    if (index2 == lastIndex && (endMs1 + durationMs1) < (_this.duration * 1000)) {
                    ...
                    _this.splitData(thisController, index2, srtData1, srtData2, splitData, splitText);
                }
            });
            return controller;
        };
        ...
        this.resetAllTimeline = function () {
            let elems = this.getElements('.srt-list-item .button-play');
            let max = elems.length;
            for (let i = 0; i < max; i++) {
        ...
        this.deleteData = function (index) {
            ...
            if(elemDrag)
            {
                ...
                // update index on UI
                let elems = _this.getElement('.srt-list-container').childNodes;
                let nodeListControllerCount = elems.length;
                let i;
                for (i = 0; i < nodeListControllerCount; i++) {
                    elems[i].setAttribute('data-index', i);
                }
                let elemTimelineNew = _this.getElement('.srt-container').childNodes;
        ...
        this.updateTextController = function (index, text) {
            // find controller
            _this.getElement('.srt-controller[data-index="' + index + '"] .srt-text-container').innerText = text;
        };
        ...
        this.placeRawData = function (text) {
            this.getElement('.srt-text-raw').value = text;
        };
        ...
        this.getControllerByIndex = function (index) {
            return this.getElement('.srt-controller[data-index="' + index + '"]');
        };

        this.getTimelineByIndex = function(index)
        {
            return this.getElement('.srt-list-item[data-index="' + index + '"]');
        }
        ...
        this.drawTimeline = function () {
            ...
            let canvas = this.getElement('.srt-timeline-canvas');
            let width = Math.round(this.msToUnit(this.duration * 1000));
            let ctx = canvas.getContext('2d');
            canvas.setAttribute('width', width);
            canvas.setAttribute('height', 40);
            this.getElement('.srt-timestamp').style.width = width + this.unit;
            this.getElement('.srt-edit-area').style.width = width + this.unit;
        ...
        this.renderWaveform = function () {
            ...
            let canvas = this.getElement(".srt-timeline-canvas-edit");
            let margin = 0;
        ...
        this.updatePointerPosition = function (left) {
            // trim container
            left = left - 3;
            this.getElement('.srt-time-position-pointer').style.left = left + this.unit;
        };
        ...
        this.onMouseWheel = function (e) {
            if (e.ctrlKey == true) {
                ...
                this.getElement('.srt-zoom-control').value = this.zoomLevelIndex;
            }
        ...
        this.initEditor = function () {
            ...
            this.getElement('.srt-zoom-control').value = this.zoomLevelIndexOriginal;
            this.getElement('.srt-zoom-control').addEventListener('change', function(e){
            ...
            this.audio.load();
            this.editorListElement = this.getElement('.srt-list-container');
            this.editorMapElement = this.getElement('.srt-map-srt-container');

            this.getElement('.srt-timeline-canvas').addEventListener('click', function (e) {
                let offset = _this.getAbsoluteOffsetLeft(e.target);
                let pos = e.clientX + _this.getElement('.srt-map').scrollLeft - offset;
            ...
            this.getElement('.srt-map').addEventListener('wheel', function (e) {
                _this.onMouseWheel(e);
            });

            this.getElement('.srt-text-raw').addEventListener('paste', function (e) {
                e.preventDefault();
                e.stopPropagation();
            ...
            document.body.addEventListener('keydown', function (e) {
                if (e.ctrlKey && window.event.keyCode == 48) {
                    ...
                    _this.getElement('.srt-zoom-control').value = _this.zoomLevelIndex;
                    let zoom = _this.zoomLevel[_this.zoomLevelIndex];
                    _this.updateZoom(zoom);
                }
            });

            let tooltipElement = _this.getElement('[data-toggle="tooltip"]');
