...
class SrtGenerator {
    constructor(editorCtrl, rawData, path) {
        ...
        {
            console.log('Create new at ', this.insertAt);
            let curMs = this.insertAt * 1000;
            let curIndex = -1;
        ...
        this.initEditor = function () {
            ...
            this.querySelector('.srt-timeline-canvas').addEventListener('contextmenu', function(e){
                ...
                let y = e.clientY - 24;
                _this.showContextMenu(x, y, seconds)
            });
            ...
            this.querySelector('.srt-time-position-pointer').addEventListener('contextmenu', function(e){
                e.preventDefault();
                e.stopPropagation();
                let offset = parseFloat(_this.closest(e.target, '.srt-time-position-pointer').style.left);
                let pos = offset;
                let seconds = _this.unitToMs(pos) / 1000;
                let x = pos + 20;
                let y = e.clientY - 24;
